    ...
    if request.status_code == 200:
        ...
        raise Exception("Query failed to run by returning code of {}. {}."
                        .format(request.status_code, json['query'],
                                json['variables']))
#Na query, mudar a primary language para que filtre outras linguagens
#Como não há um parâmetro 'stars', ele seleciona repositorios aleatórios daquela linguagem
query = """
{
  search(query:"language:javascript", type:REPOSITORY, first:10{AFTER}){
    nodes{
      ... on Repository
      {
        nameWithOwner
...
token = '' #Insira seu token da API do GitHub aqui
...
#A linha abaixo serve para testar se os dados estão sendo coletados    
#print(nodes)

#--------------Construção do CSV
#Definindo o cabeçalho
with open("repositorios.csv", 'w', newline='') as n_file:
  fnames = [
          'Nome/Dono;',
          'URL;',
          'Linguagem Primária;',
